...
import { registerGenericRecordTools } from "./generic-records.js";
import { registerMetadataWriteTools } from "./metadata-write.js";
    ...
    async (args) => {
      try {
        ...
        const companyData = {
          ...
          ...(args.idealCustomerProfile !== undefined && {
            idealCustomerProfile: args.idealCustomerProfile,
          }),
        };
    ...
    async (args) => {
      try {
        ...
        const updates = {
          ...
          ...(updateData.idealCustomerProfile !== undefined && {
            idealCustomerProfile: updateData.idealCustomerProfile,
          }),
        };
  ...
  server.tool(
    ...
    "Create a new task in Twenty CRM",
    {
      ...
      assigneeId: z
        .string()
        .optional()
        .describe("ID of the person assigned to the task"),
    },
    async (args) => {
      try {
...
export function registerRelationshipTools(
  ...
  server.tool(
    ...
    "Get all contacts (people) associated with a specific company",
    {
      companyId: z
        .string()
        .describe("The ID of the company to get contacts for"),
    },
    async (args) => {
      try {
          ...
          content: [
            {
              type: "text" as const,
              text:
                `Company Contacts for "${result.companyName}"\n` +
  ...
  server.tool(
    ...
    "Get all opportunities where a specific person is the point of contact",
    {
      personId: z
        .string()
        .describe("The ID of the person to get opportunities for"),
    },
    async (args) => {
      try {
        const result = await client.getPersonOpportunities(args.personId);

        const opportunitiesList = result.opportunities
          .map((opp) => {
          ...
          content: [
            {
              type: "text" as const,
              text:
                `Opportunities for "${result.personName}"\n` +
                `Person ID: ${result.personId}\n` +
                `Total Opportunities: ${result.totalOpportunities}\n\n` +
                (result.totalOpportunities > 0
                  ? `Opportunities:\n${opportunitiesList}`
                  : "No opportunities found for this person."),
            },
          ],
        };
      } catch (error) {
  ...
  server.tool(
    ...
    "Link an opportunity to a company and/or point of contact",
    {
      ...
      companyId: z
        .string()
        .optional()
        .describe("The ID of the company to link to (optional)"),
      pointOfContactId: z
        .string()
        .optional()
        .describe("The ID of the person to set as point of contact (optional)"),
    },
    async (args) => {
      try {
          ...
          content: [
            {
              type: "text" as const,
              text:
                `Successfully linked opportunity "${result.name}"\n` +
  ...
  server.tool(
    ...
    "Transfer a contact (person) from one company to another",
    {
      ...
      toCompanyId: z
        .string()
        .describe("The ID of the company to transfer the contact to"),
      fromCompanyId: z
        .string()
        .optional()
        .describe("The ID of the current company (optional, for validation)"),
    },
    async (args) => {
      try {
          ...
          content: [
            {
              type: "text" as const,
              text:
                `Successfully transferred contact "${result.name.firstName} ${result.name.lastName}"\n` +
  ...
  server.tool(
    ...
    "Get a summary of all relationships for a specific entity (company or person)",
    {
      entityId: z
        .string()
        .describe("The ID of the entity to get relationship summary for"),
      entityType: z
        .enum(["company", "person"])
        .describe("The type of entity (company or person)"),
    },
    async (args) => {
      try {
        const result = await client.getRelationshipSummary(
          args.entityId,
          args.entityType,
        );
          ...
          content: [
            {
              type: "text" as const,
              text:
                `Relationship Summary for ${args.entityType}: ${args.entityId}\n\n` +
...
export {
  ...
  registerGenericRecordTools,
  registerMetadataWriteTools,
};
