...
async function main() {
  ...
  const httpServer = createServer(async (req, res) => {
    ...
    try {
      ...
      // Metadata discovery (read) + dynamic record tools (custom-field aware) +
      // metadata write tools (build objects/fields). These make custom fields
      // and custom objects fully usable through the connector.
      registerMetadataTools(server, client);
      registerGenericRecordTools(server, client);
      registerMetadataWriteTools(server, client);
  ...
  httpServer.listen(port, () => {
    ...
    if (process.env.SMITHERY_CONFIG_APIKEY) {
      ...
      console.log(
        `Example: http://localhost:${port}/mcp?apiKey=YOUR_API_KEY&baseUrl=https://api.twenty.com`,
      );
    }
  });
}
